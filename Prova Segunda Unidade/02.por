programa
{
	...
	funcao inicio()
	{
		...
		{
			se (vIntercalado[i] > maior)
			{
				maior = vIntercalado[i]
			}

			se (vIntercalado[i] < menor)
			{
				menor = vIntercalado[i]
			}
			
			se (i % 2 == 0 e vIntercalado[i] % 2 != 0) // Checa se i(a posição) é par E se o valor nessa posição é impar
			{
				contImpares++
			}
		}
		...
		// Saída
		escreva("\nvIntercalado:")
		para (inteiro i=0; i<TAM*2; i++) // Lembre se que temos que usar o tamanho do vetor intercalado, e não dos normais
		{
			escreva(" ", vIntercalado[i])
		}
