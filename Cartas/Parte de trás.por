	...
	funcao inicio()
	{
		...
		enquanto(cartaoAtual<22)
		{
			...
			para(inteiro i=0; i<quantY; i++) {para(inteiro j=0; j<quantX; j++)
			{
				...
				inteiro imagem = g.carregar_imagem("heart/"+id)
			...
			delArq("img"+imagemAtual+"back.png")
			g.salvar_imagem(imagem, ("img"+imagemAtual+"back.png"))
			g.liberar_imagem(imagem)
		}
 ...
 * @POSICAO-CURSOR = 763; 
 * @PONTOS-DE-PARADA = ;
 * @SIMBOLOS-INSPECIONADOS = {cartaoAtual, 14, 10, 11}-{imagemAtual, 14, 25, 11}-{i, 18, 16, 1}-{j, 18, 50, 1};
